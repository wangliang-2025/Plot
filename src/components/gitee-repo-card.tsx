...
import { Star, GitFork, Lock, GitBranch, ExternalLink, Archive } from 'lucide-react';
import { formatDate } from '@/lib/utils';
import { languageColor, type GiteeRepo } from '@/lib/gitee';

interface Props {
  repo: GiteeRepo;
}

export function GiteeRepoCard({ repo }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 text-[11px] text-muted-foreground/80">
        {t('updated_on', { date: formatDate(new Date(repo.pushed_at || repo.updated_at), dateLocale) })}
      </div>
    </a>
  );
}
